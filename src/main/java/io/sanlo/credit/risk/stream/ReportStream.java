package io.sanlo.credit.risk.stream;
...
import io.sanlo.credit.risk.config.KafkaCompanyConfig;
import io.sanlo.credit.risk.config.KafkaFinancialMetricConfig;
import io.sanlo.credit.risk.config.KafkaFinancialReportConfig;
...
public class ReportStream {
    ...
    public KafkaStreams stream() {
        ...
        var globalKTable = financialMetrics.globalTable(environment.getProperty("io.sanlo.kafka.properties.company-topic"),
                Consumed.with(
                        Serdes.Integer(),
                        getCompanySpecificAvroSerde()
                ));
        financialMetrics.stream(
                environment.getProperty("io.sanlo.kafka.properties.financial-metric-topic"),
                Consumed.with(
                        Serdes.Integer(),
...
//                .foreach((key, value) -> log.info(format("key: %s, value: %s", key, value)))
                .to(
                        environment.getProperty("io.sanlo.kafka.properties.financial-reports"),
